...
if(sessionId && blob && blob.size > 0){ 
  ...
  useEffect(() => {
    const handleAiResponse = async (data) => {
      ...
      // Mark AI as speaking
      setActiveSpeaker('ai');
      // Normalize incoming text field (support multiple payload shapes)
      const incomingText = (data?.response ?? data?.aiText ?? data?.text ?? '').toString();
      const trimmed = incomingText.trim();

      // If AI signals interview end with exact marker [END], run full stop flow
      if (trimmed === '[END]') {
        setMessages((prev) => [...prev, { sender: 'system', text: 'AI has ended the interview.' }]);
        try {
          // ensure we wait for stopInterview to finish uploads/emits
          await stopInterview();
        } catch (err) {
          console.warn('stopInterview failed in ai-response', err);
        }
        // stopInterview already sets isInterviewActive(false) and navigates when upload finishes
        return;
      }

      // Normal AI reply: store/display the normalized text
      setMessages((prev) => [...prev, { sender: 'ai', text: trimmed }]);
 ...
 useEffect(() => {
  ...
  <div className="w-full h-full">
    ...
    <div className="text-center mb-6">
      <h1 className="text-6xl font-extrabold text-white tracking-tighter drop-shadow-lg">
        AI Interview Portal
      </h1>
  ...
  {/* Main Grid: Video area on left, Chat on right */}
  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[calc(100vh-280px)]">
    
    {/* Left side: Video feeds and controls (2 columns) */}
    <div className="lg:col-span-2 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className={`relative rounded-3xl mt-20  overflow-hidden bg-gray-900 border border-gray-700 shadow-2xl transition-all duration-300 transform hover:scale-[1.01] hover:shadow-purple-500/10 ${activeSpeaker === 'user' ? 'ring-4 ring-emerald-500 border-emerald-400 shadow-2xl' : ''}`}>
          <div className="aspect-video bg-gray-950 relative">
            {isInterviewActive ? (
              <>
                ...
                />
                {isRecording && (
                  <div className="absolute top-5 right-5 flex items-center gap-2 bg-red-600/90 px-4 py-2 rounded-full shadow-lg">
                    <div className="w-3 h-3 bg-white rounded-full animate-pulse" />
                    <span className="text-white text-sm font-semibold uppercase">Rec</span>
                  </div>
                )}
              </>
            ) : (
              <div className="w-full h-full flex items-center justify-center bg-gray-950">
                <VideoOff className="w-24 h-24 text-gray-700 opacity-60" />
              </div>
            )}
          </div>
          <div className="p-5 bg-gray-800/50 backdrop-blur-sm border-t border-gray-700">
            <div className="flex items-center justify-between">
              <span className="text-white font-bold text-xl tracking-wide">You</span>
              <div className="flex gap-3">
                {activeSpeaker === 'user' && (
                  <div className="ml-3 px-3 py-2 bg-emerald-600 text-white text-sm md:text-lg font-bold rounded-full flex items-center shadow-lg animate-pulse">Your turn</div>
                )}
                {isInterviewActive ? (
                  <>
                    <div className="p-3 bg-green-500/20 rounded-xl transition-colors duration-200">
        ...
        {/* AI Video */}
        <div className={`relative rounded-3xl mt-20 overflow-hidden bg-gray-900 border border-gray-700 shadow-2xl transition-all duration-300 transform hover:scale-[1.01] hover:shadow-purple-500/10 ${activeSpeaker === 'ai' ? 'ring-4 ring-purple-500 border-purple-400 shadow-2xl' : ''}`}>
          <div className="aspect-video relative overflow-hidden bg-gray-950">
            <div className="absolute inset-0 flex items-center justify-center">
              <div className={`relative transition-all duration-1000 ${aiPulse && isInterviewActive ? 'scale-110' : 'scale-100'}`}>
                <div className="w-48 h-48 rounded-full bg-gradient-to-br from-purple-800 to-indigo-900 flex items-center justify-center shadow-inner-lg shadow-purple-900/50">
                  <div className="w-40 h-40 rounded-full bg-gray-900 flex items-center justify-center border-4 border-purple-600/50">
                ...
                {isInterviewActive && (
                  <>
                    ...
                    <div className="absolute inset-0 rounded-full border-2 border-purple-400 animate-pulse opacity-40" />
                  </>
                )}
              </div>
            </div>
          ...
          <div className="p-5 bg-gray-800/50 backdrop-blur-sm border-t border-gray-700">
            <div className="flex items-center justify-between">
              <span className="text-white font-bold text-xl tracking-wide">AI Interviewer</span>
              <div className={`px-4 py-1.5 rounded-full ${isInterviewActive ? 'bg-green-500/20' : 'bg-gray-700/50'}`}>
                <span className={`text-sm font-semibold uppercase ${isInterviewActive ? 'text-green-400' : 'text-gray-400'}`}>
                  {isInterviewActive ? 'Active' : 'Standby'}
                </span>
              </div>
              {activeSpeaker === 'ai' && (
                <div className="ml-3 px-3 py-2 bg-purple-700 text-white text-sm md:text-lg font-bold rounded-full flex items-center shadow-lg animate-pulse">AI speaking</div>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row justify-center gap-6">
        <button
          ...
          className={`flex items-center justify-center gap-3 px-10 py-4 rounded-full font-bold text-lg shadow-lg transition-all duration-300
            ${isInterviewActive
  ...
  <div className="lg:col-span-1 flex flex-col h-full max-h-[70vh] lg:max-h-[calc(100vh-280px)]">
      <div className="bg-gray-900 rounded-3xl border border-gray-700 shadow-2xl flex flex-col h-full overflow-hidden">
        ...
        <div className="p-4 border-b border-gray-700 bg-gray-800/50 rounded-t-3xl">
          ...
          <p className="text-sm text-gray-400">Interview transcript</p>
        </div>
          ...
          {isInterviewActive ? (
            messages.length > 0 ? (
              messages.map((msg, index) => (
                ...
                >
                  <div
                    className={`p-4 rounded-3xl max-w-[85%] shadow-lg
                      ${msg.sender === 'user'
              ...
              <div className="h-full flex items-center justify-center text-gray-500">
                <p className="text-center">Conversation will appear here...</p>
              </div>
            )
          ) : (
            <div className="h-full flex items-center justify-center text-gray-500">
              <p className="text-center">Start the interview to begin chatting</p>
            </div>
          )}
        </div>
        ...
        {/* Chat Input */}
        {isInterviewActive && (
          <div className="p-4 border-t border-gray-700 bg-gray-800/50 rounded-b-3xl">
            <div className="flex items-center gap-3">
              <input
                ...
                className="flex-grow p-3 rounded-full bg-gray-800 text-white placeholder-gray-500 border border-gray-700 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-colors duration-200"
              />
              <button
                onClick={sendMessage}
                className="bg-purple-600 hover:bg-purple-700 text-white p-3 rounded-full shadow-lg transition-transform duration-200 transform hover:scale-105"
              >
                <Send className="w-5 h-5" />
              </button>
            </div>
            <p className="text-xs text-gray-500 mt-2 text-center">Voice is the primary input</p>
          </div>
        )}
      </div>
    </div>
    ...
    {isUploading && (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
        <div className="bg-gray-900 p-8 rounded-2xl flex flex-col items-center gap-4 border border-gray-700">
          <div className="w-16 h-16 border-4 border-t-transparent border-purple-500 rounded-full animate-spin" />
          <div className="text-white text-lg font-semibold">Uploading interview video...</div>
          <div className="text-gray-400 text-sm">Please wait — you'll be redirected when complete.</div>
        </div>
      </div>
    )}
